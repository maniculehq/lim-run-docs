...
`playwright-python`, `playwright-java`, and `playwright-dotnet` don't expose the Android device API. If your stack can't run Node, use the [Appium](/testing/appium) Android flow instead.
...
This is the path the [Appium](/testing/appium#run-an-android-test) page uses for UiAutomator2. It works for Playwright, but every CDP call traverses your laptop, so expect noticeably slower test runs.
...
<Cards>
  <Card title="Appium" href="/testing/appium">
