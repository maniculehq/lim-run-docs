...
<Note>
  If you're handing this off to a coding agent, read [CLI for coding agents](/agents/cli) instead. That page is the skill an agent should follow end to end.
</Note>
...
<Steps>
  ...
  <Step title="Authenticate">
    On a machine with a browser, run the login flow:
    ...
    On a headless host (CI, a devcontainer, or a cloud agent), create a token manually: open [console.limrun.com/settings](https://console.limrun.com/settings), go to **API Keys**, click **Create Token**, then export it in the environment:
