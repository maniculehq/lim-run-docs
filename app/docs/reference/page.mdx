---
title: "SDK reference"
description: "Authentication, errors, resource lifecycles, and the resources × CRUD matrix in cURL, TypeScript, Python, Go, Java, and the `lim` CLI."
order: 8
icon: book-open
---
# SDK reference

This page is the language-agnostic reference for the Limrun REST API. Each operation is shown with code-group tabs for cURL, the four official SDKs (TypeScript, Python, Go, Java), and the `lim` CLI.
...
Override with the `LIMRUN_BASE_URL` environment variable (Java SDK) or per-client option (TypeScript / Python / Go).
...
LimrunClient client = LimrunOkHttpClient.fromEnv();   // reads LIM_API_KEY + LIMRUN_BASE_URL
```
...
The org API key must never ship to a browser or end-user client. For embedded simulators, your backend holds the API key and passes only `endpointWebSocketUrl` + `status.token` to `<RemoteControl />`. See [Embed the simulator](/docs/platform/embed-simulator#authentication-model).
...
The four SDKs are not equivalent. Every one covers the control plane (create / get / list / delete instances and assets). Beyond that, only the TypeScript SDK is complete; the others vary. The CLI is the easiest fallback for anything your SDK doesn't cover; raw HTTP is the universal escape hatch.

| Capability | TypeScript | Python | Go | Java |
|---|---|---|---|---|
| Instance CRUD (iOS, Android, Xcode) | ✓ | ✓ | iOS + Android only (no `XcodeInstances` service) | iOS + Android only (no `xcodeInstances()`) |
| Asset CRUD | ✓ | ✓ | ✓ | ✓ |
| `assets.getOrUpload` (MD5 dedup + signed PUT) | ✓ | upload manually | ✓ (`Assets.GetOrUpload`) | upload manually |
| iOS device control (taps, screenshot, element-tree, typeText, simctl streaming, ...) | ✓ | use REST or CLI | ✓ (subset, see below) | use REST or CLI |
| Android device control over the SDK | ✓ | use REST + ADB | use REST + ADB | use REST + ADB |
| ADB tunnel for Android | ✓ (`startAdbTunnel`) | use external `adb` | ✓ (`tunnel.NewADB`, plus `Multiplexed`) | use external `adb` |
| Xcode source-sync + remote `xcodebuild` | ✓ (`xcodeInstances.createClient`) | use `lim xcode build` | use `lim xcode build` | use `lim xcode build` |
| iOS reverse tunnel for Detox-style flows | ✓ (`startReverseTunnel`) | not supported | not supported | not supported |
| iOS app-log / syslog streaming | ✓ | not supported | not supported | not supported |
| iOS video recording | ✓ | not supported | not supported | not supported |
| StoreKit local-config helpers | ✓ | not supported | not supported | not supported |
...
If you're on Python or Java and need any of the device-control or build surfaces, the canonical option is the `lim` CLI from `@limrun/cli` (shell out from your code) or raw REST calls against the URLs returned in `instance.status`. The CLI is feature-complete; the SDKs catch up later.

## Errors

Errors are returned as JSON with an HTTP status code that maps to a typed error class in every SDK.

| Status | TypeScript / Python / Java / Go | Meaning |
|---|---|---|
...
```java Java
// Attach an Xcode sandbox with spec.sandbox.xcode in cURL/TypeScript/Python/Go,
// or use `lim ios create --xcode`. The Java SDK may lag the OpenAPI spec for sandbox fields.
...
```java Java
IosInstanceListPage page = client.iosInstances().list(
    IosInstanceListParams.builder()
        .labelSelector("session=demo")
        .state("ready")
        .build()
);
page.items().forEach(inst -> System.out.println(inst.metadata().id()));
```
...
```java Java
IosInstance inst = client.iosInstances().get(id);
client.iosInstances().delete(id);
```
...
The Android operations have the same shape as iOS. Substitute `androidInstances` (TS / Java), `android_instances` (Python), `AndroidInstances` (Go), or `lim android *` (CLI). The Android-specific pieces are `clues` accepting `{ kind: 'OSVersion', osVersion: '13'|'14'|'15' }`, the multi-asset shapes on `initialAssets`, and `spec.sandbox.playwrightAndroid.enabled`.

### Xcode instances

The TypeScript and Python SDKs expose a top-level `xcodeInstances` / `xcode_instances` resource with the same CRUD shape as iOS. The **Go and Java SDKs do not** ship an `XcodeInstances` service today; reach the same instances through one of these paths:
...
Assets use `getOrCreate` (upsert) and `getOrUpload` (upsert plus actual upload) instead of `create`. The `os` field on `Asset` is optional; leaving it unset means the asset is available for both platforms. The **`getOrUpload` helper** that handles MD5 dedup and the signed PUT ships in the TypeScript and Go SDKs; Python and Java callers do the two-step pattern manually (call `getOrCreate`, then PUT the bytes to `signedUploadUrl` if `md5` is missing or doesn't match).
...
```java Java
AssetGetOrCreateResponse upserted = client.assets().getOrCreate(
    AssetGetOrCreateParams.builder().name("my-app.tar.gz").build()
);
List<Asset> listed  = client.assets().list(
    AssetListParams.builder().namePrefixFilter("my-app-").includeDownloadUrl(true).build()
);
Asset fetched = client.assets().get(id);
client.assets().delete(id);
```
...
`list` endpoints in TypeScript / Python / Go return iterable objects. Java returns a page object you can step through manually.
...
while (page.hasNextPage()) {
    page.items().forEach(inst -> System.out.println(inst.metadata().id()));
    page = page.getNextPage().orElseThrow();
}
```
</CodeGroup>
...
All four SDKs default to a 5-minute request timeout and 2 retries with exponential backoff. Configure both per-client with the `timeout` and `maxRetries` options, or override per-request.
