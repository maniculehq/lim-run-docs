...
| `spec.sandbox.playwrightAndroid.enabled` | boolean | Provision a Playwright-Android sub-sandbox alongside the emulator and surface its CDP URL on `status.sandbox.playwrightAndroid.url`. Defaults to `false`. See [Automated testing > Playwright](/testing/playwright). |
...
Anything you want to upload once and reuse goes through [Asset Storage](/platform/asset-storage) first. For builds you want to push to an already-running pod, see [Install an APK](#install-an-apk).
...
- For an embedded experience inside your own app, see [Embed the simulator](/platform/embed-simulator).
...
| `status.endpointWebSocketUrl` | WebSocket endpoint for the [`<RemoteControl />` web component](/platform/embed-simulator) (append `?token=`). |
| `status.signedStreamUrl` | Console-hosted watch URL with the token baked in. Open in a browser without your API key. |
| `status.mcpUrl` | Per-instance MCP HTTP endpoint. Authenticate with your **org API key**, not `status.token`. See [MCP](/agents/mcp). |
...
- **Appium.** Set `appium:udid` (or `appium:deviceName`) to `127.0.0.1:<port>` on any upstream Appium server. See [Automated testing > Appium](/testing/appium).
...
<Cards>
  <Card title="Automated testing with Appium" href="/testing/appium">
    Wire any upstream Appium server to the ADB tunnel for native or hybrid Android tests.
  </Card>
  <Card title="Automated testing with Playwright" href="/testing/playwright">
    Drive Chrome on the emulator with Playwright via the `playwrightAndroid` sub-sandbox.
  </Card>
  <Card title="Embed the simulator" href="/platform/embed-simulator">
    Stream the live emulator into your web app with `<RemoteControl />`.
  </Card>
  <Card title="Asset Storage" href="/platform/asset-storage">
