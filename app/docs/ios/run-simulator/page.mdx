...
Your iOS instance is `ready`. For provisioning, see the [Quickstart](/quickstart); for builds, see [Build with remote Xcode](/ios/build-with-xcode).
...
- **Go** covers most of the same methods. See the [SDK capability matrix](/reference#sdk-capability-matrix) for the exact list.
...
| `endpointWebSocketUrl` | WebSocket endpoint for the [`<RemoteControl />` web component](/platform/embed-simulator) (pass with `?token=`). |
| `signedStreamUrl` | Console-hosted watch URL with the token embedded; open in a browser without your API key. |
| `mcpUrl` | Per-instance MCP HTTP endpoint. Auth with the per-instance `status.token`, not your org API key. See [MCP](/agents/mcp). |
...
| `initialAssets` | array | Apps to install at boot. See [Asset Storage](/platform/asset-storage). |
| `sandbox.xcode.enabled` | boolean | Attach an Xcode build sandbox. See [Build with remote Xcode](/ios/build-with-xcode). |
...
<Cards>
  ...
  <Card title="Test in-app purchases" icon="credit-card" href="/ios/in-app-purchases">
