...
Copy that host into the commands below in place of `<host>`. Open the dev client through your app's scheme (the `scheme` in `app.json`, or `exp+<slug>` if it has none):

```bash
# Second terminal: run Metro on the matched port, advertising the tunnel host.
REACT_NATIVE_PACKAGER_HOSTNAME=<host> npx expo start --dev-client --host lan --port 57090

# Once both are running, open the dev client pointed at the tunnel.
URL="$(node -e 'console.log(encodeURIComponent("http://<host>:57090"))')"
lim ios open-url "<scheme>://expo-development-client/?url=$URL"
```
