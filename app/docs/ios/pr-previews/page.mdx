...
3. **Build on Limrun.** `lim xcode create --reuse-if-exists` provisions (or reuses) a sandbox tagged with the PR number and repo name, so subsequent pushes to the same PR reuse the same sandbox instead of spinning up a fresh one. `lim xcode build . --upload <name>` then syncs the source, runs `xcodebuild`, and uploads the artifact to [Asset Storage](/platform/asset-storage). Naming the asset after the PR means the next push overwrites it, so the preview link always points at the latest commit.
