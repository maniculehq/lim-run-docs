...
The fastest setup is three commands: install the CLI, set your API key, then run `lim skills install`. The last command copies a short operating guide into your agent's skills folder so the agent picks it up on its own. The rest of this page covers the same guide and every command the agent uses.
...
The CLI detects which coding agents you have on the machine (Claude Code, Cursor, Codex) and asks where to copy the skill. The skill is one short Markdown file. The canonical version is at [`typescript-sdk/packages/cli/skills/limrun-ios/SKILL.md`](https://github.com/limrun-inc/typescript-sdk/blob/main/packages/cli/skills/limrun-ios/SKILL.md).
...
For MCP-based device control instead of the CLI, see /docs/agents/mcp.
```
...
lim ios install-app ./MyApp.ipa                # local archive (.ipa, .zip, .tar.gz), or
