...
The only configuration today is `ChromeFlag`, used to enable Chrome's command-line on non-rooted Android emulators so [Playwright](/testing/playwright) can attach. **SDK-only**: `@limrun/cli` doesn't expose a `--chrome-flag` flag, so this entry has to come from the SDK directly.
