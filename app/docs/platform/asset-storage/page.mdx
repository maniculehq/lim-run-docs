...
For how to connect platform users to those instances, see [Embed the simulator](/platform/embed-simulator).
...
The only configuration today is `ChromeFlag`, used to enable Chrome's command-line on non-rooted Android emulators so [Playwright](/docs/testing/playwright) can attach. **SDK-only**: the `lim` CLI doesn't expose a `--chrome-flag` flag, so this entry has to come from the SDK directly.
...
(The action itself rolls the build and upload into a single `lim xcode build --upload "preview/…"` step (see [Build with remote Xcode](/ios/build-with-xcode)), but the naming convention is the same.)
...
<Note>
Android's `initialAsset` has no `launchMode` field. Boot installs only. To launch an APK after the instance is ready, drive a launch from your control code (e.g. via ADB through [`startAdbTunnel`](/android/run-emulator#adb-tunnel-the-main-door-in)).
</Note>
...
For the rest of the create-call spec (regions, clues, timeouts, sandboxes), see [Run an iOS simulator](/ios/run-simulator#configure-the-instance) and [Run an Android emulator](/android/run-emulator#configure-the-instance).
...
<Note>
`sendAsset` is TypeScript-only today. From Go or Python, drive the install via `lim android install-app` (or push the APK through the ADB tunnel; see [ADB tunnel](/android/run-emulator#adb-tunnel-the-main-door-in)).
</Note>
...
For the full field reference, see [Status URL surface](/reference#status-url-surface).
...
Pair the boot install with the [`<RemoteControl openUrl="exp://…" />`](/platform/embed-simulator#component-props) prop to land the device on the user's Expo project automatically on first paint:
